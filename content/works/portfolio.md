...
techLabel: 作成技術
tech: Next.js / React / Three.js（React Three Fiber）/ GSAP / Sass
...
process:
  ...
  - title: コーディング
    ...
    text:
      ...
      - 背景にはThree.js（React Three Fiber）を使用し、夏の大三角と奥行きの異なる星を3D空間に配置しました。スクロール量に応じてカメラを移動させることで、星の中へ進んでいくような視差表現を実装しています。また、GSAPを用いて小さな星ごとに移動速度や明滅のタイミングを変え、立体感のある夜空を表現しました。
    subCode: |
    ...
    subText: 作品情報の管理にはMarkdown形式を採用しており、Next.jsとの相性が良くデータとして扱いやすい点に加え、テキストベースで記述できるため更新が容易であり、GitHubとVercelの連携によって変更内容を自動で反映できる開発フローを構築できる点を理由に選定し、今回の制作では新たな技術として運用にも挑戦しました。
